      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Employees</p>
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white mt-1">{pendingVerification}</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-2xl flex items-center space-x-4">
            ...
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">System Admins</p>
                  ...
                  <tr key={item._id} className="hover:bg-slate-900/25 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {item._id === currentUser.id && (
                        <span className="ml-2 text-[10px] uppercase font-bold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-2 py-0.5 rounded-full">
                          You
                        </span>
                      )}
                    </td>
        ...
        <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 max-w-md w-full rounded-2xl overflow-hidden shadow-2xl p-6 animate-scaleUp">
            <div className="flex items-center space-x-3 text-amber-500 mb-4">
              <AlertCircle className="w-8 h-8 flex-shrink-0 text-indigo-500" />
              <h3 className="text-lg font-bold text-white">Confirm Security Action</h3>
            </div>
            <p className="text-slate-350 text-sm mb-6 leading-relaxed">
              Are you sure you want to {confirmModal.isVerified ? 'revoke access' : 'verify and approve access'} for <strong className="text-white">{confirmModal.userName}</strong>? This will immediately affect their system access permissions.
            </p>
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setConfirmModal({ isOpen: false, userId: null, userName: '', isVerified: false })}
                className="px-4 py-2 border border-slate-800 hover:bg-slate-800 rounded-xl text-xs font-semibold text-slate-400 hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
